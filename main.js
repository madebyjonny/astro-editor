...
ipcMain.handle("open-project-by-path", async (event, projectPath) => {
  const contentPath = path.join(projectPath, "src", "content");
...
ipcMain.handle("remove-recent-project", async (event, projectPath) => {
  const storeData = getStore();
  const recentProjects = storeData.recentProjects || [];
  const filtered = recentProjects.filter((p) => p.path !== projectPath);
...
ipcMain.handle("check-project-status", async (event, projectPath) => {
...
ipcMain.handle("start-dev-server", async (event, projectPath) => {
  ...
  if (devServers.has(projectPath)) {
    return {
      success: true,
      port: devServers.get(projectPath).port,
      alreadyRunning: true,
    };
  }
...
ipcMain.handle("get-collections", async (event, contentPath) => {
  try {
...
ipcMain.handle("get-collection-files", async (event, collectionPath) => {
  try {
...
// Save file content
ipcMain.handle("save-file", async (event, filePath, frontmatter, content) => {
  try {
...
ipcMain.handle(
  "create-file",
  async (event, collectionPath, filename, frontmatter, content) => {
    try {
...
ipcMain.handle(
  ...
  async (event, contentPath, collectionName) => {
    try {
...
function parseCollectionSchema(configContent, collectionName) {
